...
describe('Mp3FrameCounter.finalise', () => {
  ...
  it('returns the total number of frames counted', () => {
    ...
    // isolation, independent of how update() arrives at the count.
    counter.framesCount = 6089;

    expect(counter.finalise()).toBe(6089);
  });

  it('reflects state accumulated through update()', () => {
    const counter = new Mp3FrameCounter();
    counter.update(Buffer.alloc(0));
    counter.update(Buffer.alloc(0));
    counter.update(Buffer.alloc(0));

    expect(counter.finalise()).toBe(counter.framesCount);
  });

  it('is pure: repeated calls return the same value without mutating state', () => {
    const counter = new Mp3FrameCounter();
    counter.framesCount = 42;
    ...
    expect(counter.framesCount).toBe(42);
  });
});
